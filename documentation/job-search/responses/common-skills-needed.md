...
*   **Software Testing Life Cycle (STLC):** Understanding the full flow from requirement analysis to test planning, execution, and closure.
*   **Test Case Design:** Writing clear, atomic, and repeatable test cases, test plans, and test strategies.
*   **Core Testing Types:**
    *   **Functional & Regression Testing:** Ensuring new features work and old ones don't break.
    *   **Exploratory Testing:** Finding edge cases outside the "happy path".
...
*   **API Testing Tools:** **Postman** (highly requested), Swagger/OpenAPI, REST Assured (for Java-based roles, though Playwright/Cypress is preferred for JS/TS).
*   **API Concepts:** Validating REST APIs, HTTP status codes, JSON payload validation, authentication flows (tokens, OAuth).
