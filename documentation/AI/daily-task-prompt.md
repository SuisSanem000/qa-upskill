...
1. **Pull latest changes:** Before changing anything in the project, make sure you have updated changes from remote by pulling them from the server of the repository (e.g., via `git pull`).
2. **Read the project documentation** — check `documentation/project_state.md`, `documentation/roadmap.md`, and all files in `documentation/AI/` to understand exactly where I am in the program.
3. **Identify the next incomplete day** from the progress tracker in `project_state.md` (the first day marked 🔲 Not Started).
4. **Give me a concise, step-by-step overview** of what today's session covers — short and scannable (a table or bullet list), not a wall of text.
5. **Deliver the full session content** directly — do not give me a plan or ask for approval. This means:
...
6. **Update the master interview Q&A bank** — `documentation/interview-prep/interview-qa.md`:
...
7. **Update `documentation/project_state.md`**:
