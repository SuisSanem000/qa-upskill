// Day 23 — Form Interactions, File Upload & Page Objects (TypeScript)
// Target: https://the-internet.herokuapp.com
// Run: npx playwright test tests/day23-forms.spec.ts

import { test, expect, Page } from '@playwright/test';

// ============================================================
// PAGE OBJECT — LoginPage (typed with TypeScript)
...
class LoginPage {
  ...
  constructor(page: Page) {
    this.page = page;
    // Re-assign so TypeScript knows these are bound to the injected page instance
    (this as any).usernameInput = page.locator('#username');
    (this as any).passwordInput = page.locator('#password');
    (this as any).loginButton   = page.locator('button[type="submit"]');
    (this as any).flashMessage  = page.locator('#flash');
  }
...
test.describe('Form Interactions & File Upload — Day 23', () => {
  ...
  test('FORM-002 — Keyboard: type and press Enter', async ({ page }) => {
    ...
    const keysToTest: string[] = ['Tab', 'Escape', 'a'];
    for (const key of keysToTest) {
      // TODO: Click the input, press the key, check result
      void key; // Remove this line once the loop body is filled in
    }
  });
  ...
  test('FORM-003 — Drag and Drop', async ({ page }) => {
    ...
    void boxB; // Remove once dragTo is implemented
  });
  ...
  test('FORM-004 — Handle new browser window/tab', async ({ page, context }) => {
    ...
    // TODO: Assert something about the new page (it should say "New Window")
